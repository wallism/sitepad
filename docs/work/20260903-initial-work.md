...
Branch: `master`  
Mode: Builder  
Status: MILESTONE 2 COMPLETE - STOPPED BEFORE MILESTONE 3
Reviewed input: [`../design-v0.1.md`](../design-v0.1.md)
Review chain: CEO/product -> CSO/security -> Engineering
...
## Product Outcome and Scope Contract

Sitepad v0.1 is successful when its builder can demonstrate and explain one trustworthy offline-first write path. The product artifact matters because it makes the lesson observable, but screen count is not a success metric.

The four milestones in this document are the reviewed v0.1 roadmap. Milestones 1 and 2 are complete. Milestone 3 conflict resolution, Milestone 4 storage pressure, photo work, and wider screen scope remain unstarted.

The Milestone 1 evidence packet must contain:

- the Redux action sequence for a successful edit and an injected failed edit;
- the matching IndexedDB transaction result for both cases;
- a browser-reload proof that restores the committed value;
- visible proof that a failed write never reaches **On this device**;
- the exact test commands and their results.

Only after that gate passes should Milestone 2 become active. The same stop rule applies between later milestones. This keeps the project useful as a learning system: each new mechanism must earn its complexity with a visible, repeatable proof.
...
7. Milestone 1 is the active build; Milestones 2-4 remain gated roadmap until the prior milestone's evidence packet passes.
8. v0.1 uses synthetic inspection data only and is not a production field deployment.
...
This is a sequence, not a parallel backlog. Do not scaffold later milestone services merely because their interfaces are described below. Keep the next milestone legible enough that a learner can trace it end to end in one sitting.
...
The v0.1 safety claim is about local durability, not confidentiality. IndexedDB is origin-isolated browser storage, but Sitepad does not encrypt it and cannot protect records from someone who can use the same unlocked OS account and browser profile. Therefore every v0.1 fixture, note, address, photo blob, and server record must be synthetic.

| Data | v0.1 classification | Rule |
|---|---|---|
| Synthetic addresses, results, and notes | Internal test data | May be stored in Redux, IndexedDB, and the local SQLite fixture; never copy real inspection details into the build |
| Operation IDs, revisions, claim IDs, timings, and error codes | Internal diagnostics | May be logged, but must not be treated as authorization secrets |
| Base, Mine, Server snapshots and terminal responses | Sensitive-shaped synthetic content | Persist only where the flow requires it; never write field values to console, application, or test-runner logs |
| Fake photo blob | Sensitive-shaped synthetic content | Use deterministic generated bytes; do not read a real device photo |
| GPS and signatures | Restricted future data | Do not collect in v0.1; require an authentication, authorization, retention, and device-loss design before implementation |

The Milestone 2 API is a local test boundary, not a deployable public service:

- bind to loopback by default and allow only the exact local Vite origin;
- reject wildcard CORS, unexpected content types, bodies over 32 KiB, identifiers over 128 characters, notes over 2,000 characters, unknown enum values, unknown inspection fixtures, and unexpected JSON members;
- generate operation and claim IDs with a cryptographically secure UUID source, while still authorizing access from server-side identity and ownership in any future multi-user version;
- validate the request before ledger lookup or mutation so malformed input and unknown inspection IDs cannot probe stored operation IDs;
- render all notes and server messages through React's normal escaped text path; do not use HTML injection escape hatches;
- log only operation metadata, state transitions, durations, retry classifications, and stable error codes. Do not log note text, snapshots, response bodies, photo bytes, GPS, signatures, or authorization material;
- keep destructive database reset and fault-injection controls development-only, excluded from production builds, and behind an explicit confirmation.

Temporary phone testing is allowed only as a documented test session: use synthetic data, HTTPS with a trusted development certificate, an explicit client-origin allowlist, a private-network firewall rule limited to the test device where practical, and shut down the listener afterward. Do not expose the API through a tunnel or public interface. Authentication and offline session expiry remain prerequisites for any real field data.

### Threat model

| Threat | v0.1 control | Residual limit |
|---|---|---|
| Spoofing | Loopback-only API by default; exact origin; no real users or data | There is no production authentication |
| Tampering | IndexedDB transactions, server validation, request fingerprint, idempotency ledger | A user with access to the browser profile can alter local data |
| Repudiation | Immutable operation IDs and retained synthetic terminal outcomes | This is not a compliance-grade audit trail |
| Information disclosure | Synthetic-only data and payload-free logs | IndexedDB and SQLite are not application-encrypted |
| Denial of service | Bounded request and field sizes; bounded SQLite busy retry | Local learning build has no production rate-limit design |
| Elevation of privilege | Only known fixtures are accepted; no privileged routes exist | Multi-user authorization is deliberately absent |

**Security gate:** any move beyond synthetic local development stops until authenticated identity, per-inspection authorization, offline token-expiry behavior, encrypted transport, retention/deletion, device-loss response, upload validation, and security logging are designed and tested.
...
| Server tests | NUnit, NSubstitute, `Microsoft.AspNetCore.Mvc.Testing`, and a temporary SQLite database |
...
This repository is pre-build. It contains the product framing, six SVG wireframes, the component-flow image, and this reviewed work document. It has no React client, .NET server, package manifests, database schema, or test harness to reuse yet.

Milestone 1 should use the existing inspection copy and visual hierarchy from `docs/design-v0.1.md` and `docs/wireframes/02-inspection.svg`, but implement only the three-item inspection slice described here. The other wireframes are reference material for later milestones, not a request to scaffold routes or placeholder screens.
...
Persistence results carry both `inspectionId` and `revision`. A commit advances `durableRevision` monotonically. A failure affects the visible state only if no newer revision has already committed or remains queued; a stale completion or stale failure must never overwrite the projection for newer work. The per-inspection writer permits at most one active transaction and one coalesced latest snapshot waiting behind it.
...
If the Web Locks API is unavailable, v0.1 fails closed into read-only mode with an unsupported-browser message. It must not silently enable editing without its single-writer invariant.

## Milestone 1 Engineering Handoff

Milestone 1 has one entry point and one durability boundary:

```text
route opens
  -> acquire edit lock
      -> unavailable/contended -> read-only explanation
      -> acquired
          -> open + migrate IndexedDB
              -> failure -> hydration_error -> non-destructive Retry
              -> success -> hydrate Redux -> ready

tap Pass/Fail/N/A or edit note
  -> reducer updates working value + currentRevision
  -> selector renders immediately as Saving
  -> listener schedules revision after 300 ms idle
      -> Flush now cancels the wait and queues the exact revision
  -> one per-inspection writer calls IndexedDB adapter
      -> commit(revision) -> durableRevision advances
          -> current == durable -> On this device
          -> newer current exists -> remain Saving
      -> abort(revision, typed cause)
          -> current still affected -> storage_error + recoverable Retry
          -> stale result -> record diagnostic only
```
...
|---|---|---|
| React inspection feature | Three item rows, note input, visible hydration/durability states, Retry and development-only Flush controls | IndexedDB calls, timers, or network work |
| Redux inspection slice/selectors | Working snapshot, hydration state, revision counters, typed storage outcome, derived copy | Browser handles or promises |
| RTK persistence listener | Debounce, per-inspection coalescing, serialized writes, revision-correlated result actions | IndexedDB schema mechanics or UI rendering |
| IndexedDB adapter | Database open/upgrade, short transactions, fixture seed, typed failures, connection close on `versionchange` | Redux dispatch or presentation copy |
| Browser test harness | Real IndexedDB, persistent-profile reload, two-tab lock and blocked-upgrade scenarios | Production runtime behavior |

Keep Milestone 1 inside `client/`; do not create `server/` until Milestone 2 begins. Dependency injection for the adapter and clock is sufficient for failure control. Do not add a general workflow engine, repository layer, service worker, RTK Query, or network abstraction.

### Milestone 1 test map

| Path | Test level | Required assertion |
|---|---|---|
| Reducer edit | Vitest unit | Value and `currentRevision` change synchronously; state becomes saving |
| Current revision commits | Vitest unit | `durableRevision` advances and selector shows **On this device** |
| Stale commit/failure arrives | Vitest unit | Newer working state and its visible durability status are unchanged |
| Rapid note edits | Listener integration with injected clock/adapter | One active write plus one latest coalesced snapshot; final text wins |
| Write abort and Retry | Listener integration | Prior durable snapshot remains; Retry writes the newest Redux snapshot |
| Hydration open/read/upgrade failure | Adapter + component integration | Existing data is not deleted; editing stays disabled; Retry can recover |
| Successful transaction | Real-browser IndexedDB integration | Transaction completion precedes the durable action |
| Reload after **On this device** | Playwright persistent context | Exact result and note return after closing and reopening |
| Close before transaction completes | Playwright with delayed adapter boundary | UI never records a durability claim that was not committed |
| Second tab and takeover | Playwright two-page test | Second tab is read-only; reload after release reacquires then rehydrates |
| `versionchange` and blocked upgrade | Playwright two-page test | Old connection closes; blocked client stays read-only with clear copy |
| Unsupported Web Locks | Component/browser-capability test | Editing fails closed with the documented message |
| Render isolation | React Profiler test | A result change commits only the affected row plus required status surface |

Milestone 1 validation commands, once the scaffold exists:
...
Record the command output in the milestone evidence packet. A mocked IndexedDB unit test may supplement these proofs, but it cannot replace the real-browser transaction and reload tests.

### Performance guardrails

- A result tap updates Redux and paints the optimistic value without awaiting IndexedDB.
- Unaffected item rows do not re-render; only the changed row and required status/count surfaces may commit.
- The persistence listener holds at most one active transaction and one coalesced latest snapshot per inspection, so fast typing cannot create an unbounded queue.
- Transactions contain synchronous IndexedDB request setup only; image work, network work, and long computation never run inside a transaction.
- Milestone 1 reads and writes the single inspection by key. Day-list indexes, large-list virtualization, photos, caches, and background workers wait until a milestone demonstrates the need.

### Milestone 1 failure registry

| Failure | Typed outcome | User sees | Recovery | Proof |
|---|---|---|---|---|
| Database open, migration, or initial read fails | `hydration_error` with safe diagnostic code | **Couldn’t open this device’s work — Retry** | Close handles and retry without deleting records | Adapter/component integration |
| Current revision transaction aborts | `storage_error` | **Not saved — your last change needs attention** | Retry the current Redux snapshot | Listener + real-browser integration |
| Older write result arrives after a newer edit | Stale revision result | **Saving** until the newest revision commits | Ignore stale projection update | Reducer/listener unit |
| Tab loses the edit lock or never acquires it | `read_only` | **Sitepad is already open in another tab** | Close first tab, reload, rehydrate | Playwright two-page |
| Upgrade is blocked by another connection | `upgrade_blocked` | Close-other-tabs guidance | Close stale connection and retry | Playwright two-page |
| Web Locks API is missing | `unsupported_browser` | Unsupported-browser explanation | Use the target browser | Capability test |

There are no silent Milestone 1 failures: every adapter error maps to a stable internal code, a Redux outcome, visible recoverable copy, and a test. Developer diagnostics may include database name, operation, revision, and error code, but never note content.
...
Retry delays in the learning build are 1, 2, 4, 8, 16, then a 30-second cap, driven by an injectable clock. Retries continue while the app is open; five failures only make the row prominent and never authorize deletion. The coordinator arms one timer for the earliest `nextAttemptAt` and recomputes it after every attempt, foreground event, successful reachability probe, and manual retry. Manual retry sets `nextAttemptAt` to now without resetting evidence or attempt history.
...
1. Require the expected JSON content type and enforce the body-size limit before parsing.
2. Validate the closed request schema, field lengths, enum values, identifier formats, and known synthetic inspection fixture before looking up an operation.
3. Serialize the validated versioned mutation DTO to one canonical representation with fixed property order, explicit null/default values, and defined text normalization; hash those exact UTF-8 bytes. The client persists the exact request representation used for retries.
4. Look up `operationId` in the ledger.
5. If it exists with the same fingerprint, return its stored terminal response.
6. If it exists with a different fingerprint, reject the request as operation-ID reuse.
7. Load the inspection and apply any deterministic rejection fixture.
8. Compare `baseVersion`; return and store a conflict response when it is stale.
9. Apply the mutation, increment the version, and atomically store the fingerprint and terminal response with the record mutation. A valid domain rejection is also stored even though it makes no inspection mutation.

In a future authenticated system, authentication and per-inspection authorization happen before step 3 and before any ledger response. Neither an operation ID nor an inspection ID grants access.
...
**Gate:** stop until reload, delayed-write, immediate-flush, double-flush, hydration-error, write-retry, stale-result, render-isolation, secondary-tab takeover, unsupported-lock, and blocked-upgrade proofs pass.
...
- Loopback-only hosting, exact-origin CORS, closed-schema request validation, payload-free structured logs, and development-only fault controls.

**Gate:** stop until duplicate trigger, stale claim, lost local response, malformed response, oversized/unknown-field request rejection, unknown-fixture rejection, idempotent retry, log-redaction, exact-origin CORS, and concurrent SQLite request proofs pass.
...
- **Full Today schedule and all six polished screens:** breadth waits until the durability, delivery, conflict, and quota lessons pass.
- **Real photo capture, compression, upload, and partial-upload recovery:** fake deterministic bytes are enough to prove local quota behavior without introducing private media.
- **Authentication, authorization, and offline token expiry:** required before real data or deployment, but not needed for a loopback synthetic learning build.
- **Service-worker app-shell caching and optional Background Sync:** foreground correctness must stand on its own first.
- **GPS, signatures, markup, and multi-device use:** each expands the data model, privacy surface, or conflict model beyond the restricted lesson.
- **Production deployment, installability, telemetry, and support operations:** the current artifact is local and evidence-driven.
- **Amendment or export of rejected operations:** rejected evidence remains recoverable; product policy for changing or exporting it is a later decision.
- **Automatic retention or pruning:** v0.1 uses synthetic data and an explicit development reset; real retention and deletion policy belongs with authentication and production governance.
...
|---|---|
| A result tap updates its row and required status surfaces without re-rendering unaffected rows | React render test and profiler observation |
...
| Oversized, open-schema, invalid-enum, overlong-field, and unknown-inspection requests are rejected before ledger access | .NET endpoint integration tests |
| Allowed-origin requests succeed and an unlisted Origin receives no CORS grant | .NET endpoint integration tests |
| API, Redux listener, and sync diagnostics contain IDs and error codes but no note, snapshot, response-body, or blob content | Captured-log tests using marker secrets |
| A production build contains no database-reset or fault-injection controls | Client build/source assertion |
...
The milestones are a local learning build, not a production release. Run the React client and .NET API on the same development machine, with the API bound to loopback. Test the client on a real phone over HTTPS only after desktop Milestones 1–3 pass and only under the temporary synthetic-data session controls in the security boundary above. Certificate trust, LAN routing, interface binding, firewall scope, and exact CORS origin must be recorded in the evidence packet when that step begins.
...
- [x] **T8 (P1)** — Extend the client domain and IndexedDB contracts for `completing`, completed inspections, self-contained outbox operations, claim metadata, retry scheduling, terminal outcomes, and the exact versioned request representation used for retries.
- [x] **T9 (P1)** — Implement the first-click **Complete** flow: synchronously freeze editing, capture `targetRevision`, ignore duplicate clicks, flush that exact revision, then atomically mark the inspection complete and create exactly one outbox operation; restore `in_progress` without an operation if storage fails.
- [x] **T10 (P1)** — Implement the foreground sync coordinator with one shared `syncRequested` intent, an in-page mutex, atomic IndexedDB claims, 30-second leases, expired-claim recovery, injectable capped backoff, earliest-retry scheduling, and manual retry without erasing attempt history.
- [x] **T11 (P1)** — Add the client transport boundary and strict response validation; classify network, HTTP, protocol, conflict, acknowledgement, and definite-rejection outcomes without treating malformed responses as terminal domain decisions.
- [x] **T12 (P1)** — Scaffold `server/Sitepad.Api` and its test project with one .NET 10 minimal endpoint, loopback-only defaults, exact-origin CORS, the 32 KiB body limit, closed-schema and known-fixture validation, bounded fields, development-only fault controls, and payload-free structured logs.
- [x] **T13 (P1)** — Implement the SQLite inspection and idempotency ledger transaction: canonical request fingerprinting, unique operation IDs, stored terminal responses, version comparison, atomic mutation, bounded busy retry, deterministic same-ID races, and rejection of operation-ID reuse with a different fingerprint.
- [x] **T14 (P1)** — Persist client responses atomically with their inspection projection, accept them only when both operation and claim IDs still match, recover a lost local response by replaying the same operation ID, and preserve all conflicted or rejected evidence without implementing Milestone 3 resolution.
- [x] **T15 (P1)** — Add the delivery status UI, developer event strip, retry controls, and payload-safe diagnostics for completion, claim, send, retry, acknowledgement, conflict, rejection, stale response, and recovery transitions.
- [x] **T16 (P1)** — Add appropriate unit tests alongside each Milestone 2 behavior, including client reducers, selectors, completion state transitions, retry classification, response validation, server validation, canonical fingerprinting, idempotency decisions, and deterministic collaborators; keep tests isolated and cover success, failure, duplicate, and stale-result paths.
- [x] **T17 (P1)** — Add the Milestone 2 component, listener/adapter, real-browser IndexedDB and reload, .NET endpoint, and SQLite integration/concurrency proofs, plus the production-build fault-control assertion and every proof named by the Milestone 2 gate.
- [x] **T18 (P1)** — After implementation and every Milestone 2 gate pass, update `README.md` to describe the delivered client/server architecture, prerequisites, run and test commands, visible delivery states, development-only diagnostics and fault controls, and the remaining local-learning-build limits; document only behavior proven by the implementation.
- [x] **T19 (P1)** — Produce the Milestone 2 evidence packet, verify the README update, and tick off every completed Milestone 2 task; then stop without beginning conflict resolution or any other Milestone 3 work.

**Milestone 2 completed:** 2026-09-03. All gates passed; see the [`20260903-milestone-2-evidence.md`](20260903-milestone-2-evidence.md) evidence packet. Milestone 3 remains unstarted and requires separate authorization.
...
|---|---|---:|---|---|
| CEO/product | Direct review chain | 1 | CLEAR | Resolved the ambiguity between the four-milestone roadmap and the active Milestone 1 build |
| CSO/security | Direct review chain | 1 | CLEAR | Added a synthetic-data trust boundary, data classification, local API controls, threat model, and production security stop gate |
| Engineering | Direct review chain | 1 | CLEAR | Added current-state inventory, revision-race invariants, module boundaries, test and failure maps, performance constraints, and build tasks |

**VERDICT:** CEO + CSO + ENGINEERING CLEARED — ready to implement Milestone 1 only.
